 ...
 *
 * Задача 5: пересечение объектов, буфер глубин
 */
...
void initOpenGL() {
	...
	glutCreateWindow("Part 5"); //Title окна
...
void display(void) {
	...
	glVertex3f(0, 0.5, -0.3); //Верхняя вершина
	glEnd();

	//Отрисовка треугольника
	glLoadIdentity(); //Сброс матрицы в изначальное состояние
	glTranslatef(0, 0, 0.2); //Перемещение вдоль оси OX
	glBegin(GL_TRIANGLES);
